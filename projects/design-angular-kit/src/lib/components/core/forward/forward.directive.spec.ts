...
describe('ItForwardDirective', () => {
  ...
  it('should trigger this.document.querySelector(...)?.scrollIntoView() if i pass a string', () => {
    component.directives.changes.subscribe(
      () => {
        spyOn(component.directives.first['document'].querySelector(<string>component.directives.first.itForward), 'scrollIntoView').and.returnValue();
        const aElement = fixture.debugElement.query(By.css('#firstA'));
        aElement.nativeElement.click();
        expect(component.directives.first['document'].querySelector(<string>component.directives.first.itForward).scrollIntoView).toHaveBeenCalled();
      }
    );
  });
